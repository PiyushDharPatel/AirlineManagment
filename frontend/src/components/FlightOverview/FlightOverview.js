...
const FlightOverview = () => {
  ...
  const check = () => {
    ...
    api.post("/api/flights", { id: id }).then((res) => {
      console.log(res.data); const flight = {
      ...
      api.post("/api/seats", { id: res.data.flight_id }).then((res) => {
        if (res.data.success) { setSeats(res.data.seats); setStype(res.data.seatClass) }; if (way === 'true') {
          api.post("/api/flights", { id: df }).then((res) => {
            const flight = {
              flight_number: res.data.flight_id,
              airline: res.data.flight_name,
              departure_airport: res.data.fromTitle,
  ...
  const handlePay = (data) => {
    ...
    if (way === 'true') {
      ...
      api.post('/api/settravel', { username: username, passengers: oldlist, flightDetails: flight_details, paymentDetails: data })
        .then((res) => {
          if (res.data.success) {
            api.post('/api/settravel', { username: username, passengers: passenger_info_list, flightDetails: arrflight, paymentDetails: data })
